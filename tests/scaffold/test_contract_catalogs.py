...
def _schema_reservation(self):
    unit = os.environ.get("SIT_PHASE_UNIT", "P2-W01")
    if unit.startswith("P3-"):
        phase_guard.phase3_unit_number(unit)
        bundle_delivered = True
    else:
        bundle_delivered = phase_guard.unit_number(unit) >= 2
    ...
    for name, contract in (("bundle", "sit-bundle/0.1"), ("report", "sit-report/0.1")):
        ...
        if name == "report" or not bundle_delivered:

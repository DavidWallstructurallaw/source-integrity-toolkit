...
def test_each_unit_has_exact_four_common_records_and_approved_paths(step):
    ...
    if step == 9:
        effective |= W09_R01_PATHS
        assert len(expected) == 7 and len(effective) == 10
        assert effective - expected == W09_R01_PATHS
    if step == 11:
...
def test_w04_r01_literal_authority_preserves_plan_guard_and_workflow():
    ...
    for path, digest in (
        ...
        if path == ".github/workflows/phase1-ci.yml":
...
def test_w05_r01_only_three_old_transition_methods_have_authorized_adaptations():
    ...
    live = ast.parse(git_bytes(path, W06_R01_PREDECESSOR))
    ...
    checkpoint = git_bytes(path, W09_R01_BASE)
    assert hashlib.sha256(checkpoint).hexdigest() == "fb67b75edab831c9fc96162eada7c760c07c8f9d0eadf1e30b06cb4934198285"
    expected = checkpoint.decode()
    for previous, replacement in (('def test_each_unit_has_exact_four_common_records_and_approved_paths(step):\n    paths = guard.phase3_plan_paths(ROOT)\n    ci = ci_driver()\n    unit = f"P3-W{step:02}"\n    expected = COMMON | ADDITIONAL[step]\n    assert set(paths) == {f"P3-W{number:02}" for number in range(1, 16)}\n    assert paths[unit] == expected\n    effective = expected | (W04_R01_PATHS if step == 4 else frozenset())\n    if step == 5:\n        effective |= W05_R01_PATHS\n    if step == 6:\n        effective |= W06_R01_PATHS\n    if step == 8:\n        effective |= W08_R01_PATHS\n    assert ci.phase3_effective_paths(paths, unit) == effective\n    ci.phase3_check_changed_paths(paths, unit, effective)\n    cumulative = COMMON | frozenset().union(*(ADDITIONAL[number] for number in range(1, step + 1)))\n    if step >= 4:\n        cumulative |= W04_R01_PATHS\n    if step >= 5:\n        cumulative |= W05_R01_PATHS\n    if step >= 6:\n        cumulative |= W06_R01_PATHS\n    if step >= 8:\n        cumulative |= W08_R01_PATHS\n    assert ci.phase3_effective_paths(paths, unit, cumulative=True) == cumulative\n    assert len(COMMON) == 4\n    for future_path in (COMMON | frozenset().union(*ADDITIONAL.values())) - effective:\n        with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\n            ci.phase3_check_changed_paths(paths, unit, {future_path})', 'def test_each_unit_has_exact_four_common_records_and_approved_paths(step):\n    paths = guard.phase3_plan_paths(ROOT)\n    ci = ci_driver()\n    unit = f"P3-W{step:02}"\n    expected = COMMON | ADDITIONAL[step]\n    assert set(paths) == {f"P3-W{number:02}" for number in range(1, 16)}\n    assert paths[unit] == expected\n    effective = expected | (W04_R01_PATHS if step == 4 else frozenset())\n    if step == 5:\n        effective |= W05_R01_PATHS\n    if step == 6:\n        effective |= W06_R01_PATHS\n    if step == 8:\n        effective |= W08_R01_PATHS\n    if step == 9:\n        effective |= W09_R01_PATHS\n        assert len(expected) == 7 and len(effective) == 10\n        assert effective - expected == W09_R01_PATHS\n    assert ci.phase3_effective_paths(paths, unit) == effective\n    ci.phase3_check_changed_paths(paths, unit, effective)\n    cumulative = COMMON | frozenset().union(*(ADDITIONAL[number] for number in range(1, step + 1)))\n    if step >= 4:\n        cumulative |= W04_R01_PATHS\n    if step >= 5:\n        cumulative |= W05_R01_PATHS\n    if step >= 6:\n        cumulative |= W06_R01_PATHS\n    if step >= 8:\n        cumulative |= W08_R01_PATHS\n    if step >= 9:\n        cumulative |= W09_R01_PATHS\n    assert ci.phase3_effective_paths(paths, unit, cumulative=True) == cumulative\n    assert len(COMMON) == 4\n    for future_path in (COMMON | frozenset().union(*ADDITIONAL.values())) - effective:\n        with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\n            ci.phase3_check_changed_paths(paths, unit, {future_path})'), ('def test_w04_r01_literal_authority_preserves_plan_guard_and_workflow():\n    ci = ci_driver()\n    assert ci.P3_W04_R01_PATHS == W04_R01_PATHS and len(W04_R01_PATHS) == 4\n    assert ci.P3_W04_R01_BASE == W04_R01_BASE\n    assert ci.P3_W04_R01_BASE_TREE == W04_R01_BASE_TREE\n    assert ci.P3_W04_R01_PREDECESSOR == W04_R01_PREDECESSOR\n    paths = guard.phase3_plan_paths(ROOT)\n    assert len(paths["P3-W04"]) == 13\n    assert len(ci.phase3_effective_paths(paths, "P3-W04")) == 17\n    assert len(ci.phase3_effective_paths(paths, "P3-W01")) == 23\n    # Original W01 permission already contains these paths; R01 gives no new\n    # retrospective W01 permission and cannot enlarge raw plan parsing.\n    assert W04_R01_PATHS <= paths["P3-W01"]\n    assert not W04_R01_PATHS & paths["P3-W04"]\n    for path, digest in (\n        ("tools/check_scaffold_boundary.py", "fc9885337b4973a084c4c7326370a7962b898c14f6a0858e09ee6ccbeb97071a"),\n        (".github/workflows/phase1-ci.yml", "841c39f7d3c596eafd767355a4a30f5111d6a73c8752229108708fcb39c4539e"),\n        ("PHASE_3_PLAN.md", PLAN_SHA256),\n        ("phase3/entry_manifest.json", "dfe96f57a222937131e8e4fafd13098f76d134cd88a50ffa9fa82891b9b9659b"),\n    ):\n        assert hashlib.sha256((ROOT / path).read_bytes()).hexdigest() == digest, path', 'def test_w04_r01_literal_authority_preserves_plan_guard_and_workflow():\n    ci = ci_driver()\n    assert ci.P3_W04_R01_PATHS == W04_R01_PATHS and len(W04_R01_PATHS) == 4\n    assert ci.P3_W04_R01_BASE == W04_R01_BASE\n    assert ci.P3_W04_R01_BASE_TREE == W04_R01_BASE_TREE\n    assert ci.P3_W04_R01_PREDECESSOR == W04_R01_PREDECESSOR\n    paths = guard.phase3_plan_paths(ROOT)\n    assert len(paths["P3-W04"]) == 13\n    assert len(ci.phase3_effective_paths(paths, "P3-W04")) == 17\n    assert len(ci.phase3_effective_paths(paths, "P3-W01")) == 23\n    # Original W01 permission already contains these paths; R01 gives no new\n    # retrospective W01 permission and cannot enlarge raw plan parsing.\n    assert W04_R01_PATHS <= paths["P3-W01"]\n    assert not W04_R01_PATHS & paths["P3-W04"]\n    for path, digest in (\n        ("tools/check_scaffold_boundary.py", "fc9885337b4973a084c4c7326370a7962b898c14f6a0858e09ee6ccbeb97071a"),\n        (".github/workflows/phase1-ci.yml", "841c39f7d3c596eafd767355a4a30f5111d6a73c8752229108708fcb39c4539e"),\n        ("PHASE_3_PLAN.md", PLAN_SHA256),\n        ("phase3/entry_manifest.json", "dfe96f57a222937131e8e4fafd13098f76d134cd88a50ffa9fa82891b9b9659b"),\n    ):\n        actual = (ROOT / path).read_bytes()\n        if path == ".github/workflows/phase1-ci.yml":\n            historical = git_bytes(path, W09_R01_BASE)\n            assert hashlib.sha256(historical).hexdigest() == digest, path\n            assert historical.count(b\'      "timeout-minutes": 25,\\n\') == 1\n            assert actual == historical.replace(b\'      "timeout-minutes": 25,\\n\', b\'      "timeout-minutes": 40,\\n\', 1)\n        else:\n            assert hashlib.sha256(actual).hexdigest() == digest, path'), ('def test_w04_r01_name_is_cumulative_but_immediate_permissions_are_not(step):\n    ci = ci_driver()\n    unit = f"P3-W{step:02}"\n    paths = guard.phase3_plan_paths(ROOT)\n    assert ci.phase3_scope_exceptions(unit) == ((["P3-W04-R01"] if step >= 4 else []) +\n                                              (["P3-W05-R01"] if step >= 5 else []) +\n                                              (["P3-W06-R01"] if step >= 6 else []) +\n                                              (["P3-W08-R01"] if step >= 8 else []))\n    for path in W04_R01_PATHS:\n        # Overlapping paths require their own approved immediate-unit scope.\n        if (step in (1, 4) or (step == 5 and path in W05_R01_PATHS) or\n                (step == 6 and path in W06_R01_PATHS) or (step == 8 and path in W08_R01_PATHS)):\n            ci.phase3_check_changed_paths(paths, unit, {path})\n        else:\n            with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\n                ci.phase3_check_changed_paths(paths, unit, {path})\n    # All four were in W01 already; the cumulative union is still independently\n    # determined by approved scopes, with no new caller/source metadata input.\n    assert W04_R01_PATHS <= ci.phase3_effective_paths(paths, unit, cumulative=True)', 'def test_w04_r01_name_is_cumulative_but_immediate_permissions_are_not(step):\n    ci = ci_driver()\n    unit = f"P3-W{step:02}"\n    paths = guard.phase3_plan_paths(ROOT)\n    assert ci.phase3_scope_exceptions(unit) == ((["P3-W04-R01"] if step >= 4 else []) +\n                                              (["P3-W05-R01"] if step >= 5 else []) +\n                                              (["P3-W06-R01"] if step >= 6 else []) +\n                                              (["P3-W08-R01"] if step >= 8 else []) +\n                                              (["P3-W09-R01"] if step >= 9 else []))\n    for path in W04_R01_PATHS:\n        # Overlapping paths require their own approved immediate-unit scope.\n        if (step in (1, 4) or (step == 5 and path in W05_R01_PATHS) or\n                (step == 6 and path in W06_R01_PATHS) or (step == 8 and path in W08_R01_PATHS) or\n                (step == 9 and path in W09_R01_PATHS)):\n            ci.phase3_check_changed_paths(paths, unit, {path})\n        else:\n            with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\n                ci.phase3_check_changed_paths(paths, unit, {path})\n    # All four were in W01 already; the cumulative union is still independently\n    # determined by approved scopes, with no new caller/source metadata input.\n    assert W04_R01_PATHS <= ci.phase3_effective_paths(paths, unit, cumulative=True)')):
        assert expected.count(previous) == 1, "invalid_fixed_w09_transition_delta"
        expected = expected.replace(previous, replacement, 1)
    original_names = {node.name for node in original if isinstance(node, ast.FunctionDef)}
    ...
    for node in original:
        assert ast.dump(current_functions[node.name]) == ast.dump(expected_functions[node.name]), node.name
...
def test_w06_r01_exact_authority_and_current_source_keep_every_other_unit_scope():
    ...
    assert ci.P3_W09_R01_PATHS == W09_R01_PATHS and len(W09_R01_PATHS) == 3
    assert ci.P3_W09_R01_BASE == W09_R01_BASE
    assert ci.P3_W09_R01_BASE_TREE == W09_R01_BASE_TREE
    assert ci.P3_W09_R01_PREDECESSOR == W09_R01_PREDECESSOR
    assert ci.git_text("rev-parse", W09_R01_BASE + "^{tree}") == W09_R01_BASE_TREE
    assert ci.git_text("show", "-s", "--format=%P", W09_R01_BASE).split() == ["59a25116bfe5fe8f4f9b407852bb29fa692e542d"]
    assert len(paths["P3-W09"]) == 7
    assert len(ci.phase3_effective_paths(paths, "P3-W09")) == 10
    history = ci.phase3_history(guard.phase3_entry_manifest(ROOT), W09_R01_PREDECESSOR,
                               W09_R01_BASE, paths, "P3-W09")
    original = [row for row in history if row["current_unit_segment"]]
    assert {row["commit"] for row in original} == {"59a25116bfe5fe8f4f9b407852bb29fa692e542d", W09_R01_BASE}
    assert all(row["immediate_scope_exceptions"] == [] and set(row["changed_paths"]) <= paths["P3-W09"] for row in original)
    assert ci.P3_W11_R01_PATHS == W11_R01_PATHS and len(W11_R01_PATHS) == 3
...
def test_w06_r01_only_seven_old_transition_methods_change_and_every_other_source_node_is_exact():
    ...
    checkpoint = git_bytes(path, W09_R01_BASE)
    assert hashlib.sha256(checkpoint).hexdigest() == "fb67b75edab831c9fc96162eada7c760c07c8f9d0eadf1e30b06cb4934198285"
    old, live = before.decode(), checkpoint.decode()
    ...
    expected = live
    for prior, replacement in (('def test_each_unit_has_exact_four_common_records_and_approved_paths(step):\n    paths = guard.phase3_plan_paths(ROOT)\n    ci = ci_driver()\n    unit = f"P3-W{step:02}"\n    expected = COMMON | ADDITIONAL[step]\n    assert set(paths) == {f"P3-W{number:02}" for number in range(1, 16)}\n    assert paths[unit] == expected\n    effective = expected | (W04_R01_PATHS if step == 4 else frozenset())\n    if step == 5:\n        effective |= W05_R01_PATHS\n    if step == 6:\n        effective |= W06_R01_PATHS\n    if step == 8:\n        effective |= W08_R01_PATHS\n    assert ci.phase3_effective_paths(paths, unit) == effective\n    ci.phase3_check_changed_paths(paths, unit, effective)\n    cumulative = COMMON | frozenset().union(*(ADDITIONAL[number] for number in range(1, step + 1)))\n    if step >= 4:\n        cumulative |= W04_R01_PATHS\n    if step >= 5:\n        cumulative |= W05_R01_PATHS\n    if step >= 6:\n        cumulative |= W06_R01_PATHS\n    if step >= 8:\n        cumulative |= W08_R01_PATHS\n    assert ci.phase3_effective_paths(paths, unit, cumulative=True) == cumulative\n    assert len(COMMON) == 4\n    for future_path in (COMMON | frozenset().union(*ADDITIONAL.values())) - effective:\n        with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\n            ci.phase3_check_changed_paths(paths, unit, {future_path})', 'def test_each_unit_has_exact_four_common_records_and_approved_paths(step):\n    paths = guard.phase3_plan_paths(ROOT)\n    ci = ci_driver()\n    unit = f"P3-W{step:02}"\n    expected = COMMON | ADDITIONAL[step]\n    assert set(paths) == {f"P3-W{number:02}" for number in range(1, 16)}\n    assert paths[unit] == expected\n    effective = expected | (W04_R01_PATHS if step == 4 else frozenset())\n    if step == 5:\n        effective |= W05_R01_PATHS\n    if step == 6:\n        effective |= W06_R01_PATHS\n    if step == 8:\n        effective |= W08_R01_PATHS\n    if step == 9:\n        effective |= W09_R01_PATHS\n        assert len(expected) == 7 and len(effective) == 10\n        assert effective - expected == W09_R01_PATHS\n    assert ci.phase3_effective_paths(paths, unit) == effective\n    ci.phase3_check_changed_paths(paths, unit, effective)\n    cumulative = COMMON | frozenset().union(*(ADDITIONAL[number] for number in range(1, step + 1)))\n    if step >= 4:\n        cumulative |= W04_R01_PATHS\n    if step >= 5:\n        cumulative |= W05_R01_PATHS\n    if step >= 6:\n        cumulative |= W06_R01_PATHS\n    if step >= 8:\n        cumulative |= W08_R01_PATHS\n    if step >= 9:\n        cumulative |= W09_R01_PATHS\n    assert ci.phase3_effective_paths(paths, unit, cumulative=True) == cumulative\n    assert len(COMMON) == 4\n    for future_path in (COMMON | frozenset().union(*ADDITIONAL.values())) - effective:\n        with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\n            ci.phase3_check_changed_paths(paths, unit, {future_path})'), ('def test_w04_r01_literal_authority_preserves_plan_guard_and_workflow():\n    ci = ci_driver()\n    assert ci.P3_W04_R01_PATHS == W04_R01_PATHS and len(W04_R01_PATHS) == 4\n    assert ci.P3_W04_R01_BASE == W04_R01_BASE\n    assert ci.P3_W04_R01_BASE_TREE == W04_R01_BASE_TREE\n    assert ci.P3_W04_R01_PREDECESSOR == W04_R01_PREDECESSOR\n    paths = guard.phase3_plan_paths(ROOT)\n    assert len(paths["P3-W04"]) == 13\n    assert len(ci.phase3_effective_paths(paths, "P3-W04")) == 17\n    assert len(ci.phase3_effective_paths(paths, "P3-W01")) == 23\n    # Original W01 permission already contains these paths; R01 gives no new\n    # retrospective W01 permission and cannot enlarge raw plan parsing.\n    assert W04_R01_PATHS <= paths["P3-W01"]\n    assert not W04_R01_PATHS & paths["P3-W04"]\n    for path, digest in (\n        ("tools/check_scaffold_boundary.py", "fc9885337b4973a084c4c7326370a7962b898c14f6a0858e09ee6ccbeb97071a"),\n        (".github/workflows/phase1-ci.yml", "841c39f7d3c596eafd767355a4a30f5111d6a73c8752229108708fcb39c4539e"),\n        ("PHASE_3_PLAN.md", PLAN_SHA256),\n        ("phase3/entry_manifest.json", "dfe96f57a222937131e8e4fafd13098f76d134cd88a50ffa9fa82891b9b9659b"),\n    ):\n        assert hashlib.sha256((ROOT / path).read_bytes()).hexdigest() == digest, path', 'def test_w04_r01_literal_authority_preserves_plan_guard_and_workflow():\n    ci = ci_driver()\n    assert ci.P3_W04_R01_PATHS == W04_R01_PATHS and len(W04_R01_PATHS) == 4\n    assert ci.P3_W04_R01_BASE == W04_R01_BASE\n    assert ci.P3_W04_R01_BASE_TREE == W04_R01_BASE_TREE\n    assert ci.P3_W04_R01_PREDECESSOR == W04_R01_PREDECESSOR\n    paths = guard.phase3_plan_paths(ROOT)\n    assert len(paths["P3-W04"]) == 13\n    assert len(ci.phase3_effective_paths(paths, "P3-W04")) == 17\n    assert len(ci.phase3_effective_paths(paths, "P3-W01")) == 23\n    # Original W01 permission already contains these paths; R01 gives no new\n    # retrospective W01 permission and cannot enlarge raw plan parsing.\n    assert W04_R01_PATHS <= paths["P3-W01"]\n    assert not W04_R01_PATHS & paths["P3-W04"]\n    for path, digest in (\n        ("tools/check_scaffold_boundary.py", "fc9885337b4973a084c4c7326370a7962b898c14f6a0858e09ee6ccbeb97071a"),\n        (".github/workflows/phase1-ci.yml", "841c39f7d3c596eafd767355a4a30f5111d6a73c8752229108708fcb39c4539e"),\n        ("PHASE_3_PLAN.md", PLAN_SHA256),\n        ("phase3/entry_manifest.json", "dfe96f57a222937131e8e4fafd13098f76d134cd88a50ffa9fa82891b9b9659b"),\n    ):\n        actual = (ROOT / path).read_bytes()\n        if path == ".github/workflows/phase1-ci.yml":\n            historical = git_bytes(path, W09_R01_BASE)\n            assert hashlib.sha256(historical).hexdigest() == digest, path\n            assert historical.count(b\'      "timeout-minutes": 25,\\n\') == 1\n            assert actual == historical.replace(b\'      "timeout-minutes": 25,\\n\', b\'      "timeout-minutes": 40,\\n\', 1)\n        else:\n            assert hashlib.sha256(actual).hexdigest() == digest, path'), ('def test_w04_r01_name_is_cumulative_but_immediate_permissions_are_not(step):\n    ci = ci_driver()\n    unit = f"P3-W{step:02}"\n    paths = guard.phase3_plan_paths(ROOT)\n    assert ci.phase3_scope_exceptions(unit) == ((["P3-W04-R01"] if step >= 4 else []) +\n                                              (["P3-W05-R01"] if step >= 5 else []) +\n                                              (["P3-W06-R01"] if step >= 6 else []) +\n                                              (["P3-W08-R01"] if step >= 8 else []))\n    for path in W04_R01_PATHS:\n        # Overlapping paths require their own approved immediate-unit scope.\n        if (step in (1, 4) or (step == 5 and path in W05_R01_PATHS) or\n                (step == 6 and path in W06_R01_PATHS) or (step == 8 and path in W08_R01_PATHS)):\n            ci.phase3_check_changed_paths(paths, unit, {path})\n        else:\n            with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\n                ci.phase3_check_changed_paths(paths, unit, {path})\n    # All four were in W01 already; the cumulative union is still independently\n    # determined by approved scopes, with no new caller/source metadata input.\n    assert W04_R01_PATHS <= ci.phase3_effective_paths(paths, unit, cumulative=True)', 'def test_w04_r01_name_is_cumulative_but_immediate_permissions_are_not(step):\n    ci = ci_driver()\n    unit = f"P3-W{step:02}"\n    paths = guard.phase3_plan_paths(ROOT)\n    assert ci.phase3_scope_exceptions(unit) == ((["P3-W04-R01"] if step >= 4 else []) +\n                                              (["P3-W05-R01"] if step >= 5 else []) +\n                                              (["P3-W06-R01"] if step >= 6 else []) +\n                                              (["P3-W08-R01"] if step >= 8 else []) +\n                                              (["P3-W09-R01"] if step >= 9 else []))\n    for path in W04_R01_PATHS:\n        # Overlapping paths require their own approved immediate-unit scope.\n        if (step in (1, 4) or (step == 5 and path in W05_R01_PATHS) or\n                (step == 6 and path in W06_R01_PATHS) or (step == 8 and path in W08_R01_PATHS) or\n                (step == 9 and path in W09_R01_PATHS)):\n            ci.phase3_check_changed_paths(paths, unit, {path})\n        else:\n            with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\n                ci.phase3_check_changed_paths(paths, unit, {path})\n    # All four were in W01 already; the cumulative union is still independently\n    # determined by approved scopes, with no new caller/source metadata input.\n    assert W04_R01_PATHS <= ci.phase3_effective_paths(paths, unit, cumulative=True)'), ('def test_w05_r01_exact_authority_preserves_plan_and_other_unit_scopes():\n    ci = ci_driver()\n    assert ci.P3_W05_R01_PATHS == W05_R01_PATHS\n    assert ci.P3_W05_R01_BASE == W05_R01_BASE\n    assert ci.P3_W05_R01_BASE_TREE == W05_R01_BASE_TREE\n    assert ci.P3_W05_R01_PREDECESSOR == W05_R01_PREDECESSOR\n    _assert_w06_r01_driver_source((ROOT / "tests/scaffold/test_ci_contract.py").read_bytes())\n    paths = guard.phase3_plan_paths(ROOT)\n    assert paths["P3-W05"] == COMMON | ADDITIONAL[5] and len(paths["P3-W05"]) == 9\n    assert len(ci.phase3_effective_paths(paths, "P3-W05")) == 13\n    assert W05_R01_PATHS <= paths["P3-W01"] and not W05_R01_PATHS & paths["P3-W05"]\n    assert W05_R01_PATHS - W04_R01_PATHS == {"tests/scaffold/test_module_manifest.py"}\n    assert W04_R01_PATHS - W05_R01_PATHS == {"tests/scaffold/test_no_runtime_implementation.py"}\n    for path, digest in (\n        ("tools/check_scaffold_boundary.py", "fc9885337b4973a084c4c7326370a7962b898c14f6a0858e09ee6ccbeb97071a"),\n        (".github/workflows/phase1-ci.yml", "841c39f7d3c596eafd767355a4a30f5111d6a73c8752229108708fcb39c4539e"),\n        ("PHASE_3_PLAN.md", PLAN_SHA256),\n        ("phase3/entry_manifest.json", "dfe96f57a222937131e8e4fafd13098f76d134cd88a50ffa9fa82891b9b9659b"),\n    ):\n        assert hashlib.sha256((ROOT / path).read_bytes()).hexdigest() == digest, path\n    assert_live_migration_bytes({p: (ROOT / p).read_bytes() for p in LIVE_MIGRATION_SHA256})', 'def test_w05_r01_exact_authority_preserves_plan_and_other_unit_scopes():\n    ci = ci_driver()\n    assert ci.P3_W05_R01_PATHS == W05_R01_PATHS\n    assert ci.P3_W05_R01_BASE == W05_R01_BASE\n    assert ci.P3_W05_R01_BASE_TREE == W05_R01_BASE_TREE\n    assert ci.P3_W05_R01_PREDECESSOR == W05_R01_PREDECESSOR\n    _assert_w06_r01_driver_source((ROOT / "tests/scaffold/test_ci_contract.py").read_bytes())\n    paths = guard.phase3_plan_paths(ROOT)\n    assert paths["P3-W05"] == COMMON | ADDITIONAL[5] and len(paths["P3-W05"]) == 9\n    assert len(ci.phase3_effective_paths(paths, "P3-W05")) == 13\n    assert W05_R01_PATHS <= paths["P3-W01"] and not W05_R01_PATHS & paths["P3-W05"]\n    assert W05_R01_PATHS - W04_R01_PATHS == {"tests/scaffold/test_module_manifest.py"}\n    assert W04_R01_PATHS - W05_R01_PATHS == {"tests/scaffold/test_no_runtime_implementation.py"}\n    for path, digest in (\n        ("tools/check_scaffold_boundary.py", "fc9885337b4973a084c4c7326370a7962b898c14f6a0858e09ee6ccbeb97071a"),\n        (".github/workflows/phase1-ci.yml", "841c39f7d3c596eafd767355a4a30f5111d6a73c8752229108708fcb39c4539e"),\n        ("PHASE_3_PLAN.md", PLAN_SHA256),\n        ("phase3/entry_manifest.json", "dfe96f57a222937131e8e4fafd13098f76d134cd88a50ffa9fa82891b9b9659b"),\n    ):\n        actual = (ROOT / path).read_bytes()\n        if path == ".github/workflows/phase1-ci.yml":\n            historical = git_bytes(path, W09_R01_BASE)\n            assert hashlib.sha256(historical).hexdigest() == digest, path\n            assert historical.count(b\'      "timeout-minutes": 25,\\n\') == 1\n            assert actual == historical.replace(b\'      "timeout-minutes": 25,\\n\', b\'      "timeout-minutes": 40,\\n\', 1)\n        else:\n            assert hashlib.sha256(actual).hexdigest() == digest, path\n    assert_live_migration_bytes({p: (ROOT / p).read_bytes() for p in LIVE_MIGRATION_SHA256})'), ('def test_w05_r01_cumulative_name_never_grants_later_immediate_rights(step):\n    ci = ci_driver()\n    paths = guard.phase3_plan_paths(ROOT)\n    unit = f"P3-W{step:02}"\n    expected = COMMON | ADDITIONAL[step]\n    if step == 4:\n        expected |= W04_R01_PATHS\n    if step == 5:\n        expected |= W05_R01_PATHS\n    if step == 6:\n        expected |= W06_R01_PATHS\n    if step == 8:\n        expected |= W08_R01_PATHS\n    assert ci.phase3_effective_paths(paths, unit) == expected\n    assert ci.phase3_scope_exceptions(unit) == ((["P3-W04-R01"] if step >= 4 else []) +\n                                              (["P3-W05-R01"] if step >= 5 else []) +\n                                              (["P3-W06-R01"] if step >= 6 else []) +\n                                              (["P3-W08-R01"] if step >= 8 else []))\n    ci.phase3_check_changed_paths(paths, unit, expected)\n    for path in (W05_R01_PATHS | W04_R01_PATHS | W06_R01_PATHS | W08_R01_PATHS) - expected:\n        with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\n            ci.phase3_check_changed_paths(paths, unit, expected | {path})\n    # These paths have been cumulative since W01. That never enlarges a later\n    # immediate scope, including W06 after the new repair is in its history.\n    assert W05_R01_PATHS <= ci.phase3_effective_paths(paths, unit, cumulative=True)', 'def test_w05_r01_cumulative_name_never_grants_later_immediate_rights(step):\n    ci = ci_driver()\n    paths = guard.phase3_plan_paths(ROOT)\n    unit = f"P3-W{step:02}"\n    expected = COMMON | ADDITIONAL[step]\n    if step == 4:\n        expected |= W04_R01_PATHS\n    if step == 5:\n        expected |= W05_R01_PATHS\n    if step == 6:\n        expected |= W06_R01_PATHS\n    if step == 8:\n        expected |= W08_R01_PATHS\n    if step == 9:\n        expected |= W09_R01_PATHS\n    assert ci.phase3_effective_paths(paths, unit) == expected\n    assert ci.phase3_scope_exceptions(unit) == ((["P3-W04-R01"] if step >= 4 else []) +\n                                              (["P3-W05-R01"] if step >= 5 else []) +\n                                              (["P3-W06-R01"] if step >= 6 else []) +\n                                              (["P3-W08-R01"] if step >= 8 else []) +\n                                              (["P3-W09-R01"] if step >= 9 else []))\n    ci.phase3_check_changed_paths(paths, unit, expected)\n    for path in (W05_R01_PATHS | W04_R01_PATHS | W06_R01_PATHS | W08_R01_PATHS | W09_R01_PATHS) - expected:\n        with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\n            ci.phase3_check_changed_paths(paths, unit, expected | {path})\n    # These paths have been cumulative since W01. That never enlarges a later\n    # immediate scope, including W06 after the new repair is in its history.\n    assert W05_R01_PATHS <= ci.phase3_effective_paths(paths, unit, cumulative=True)'), ('def test_w05_r01_only_three_old_transition_methods_have_authorized_adaptations():\n    path = "tests/contract/test_phase3_transition.py"\n    before = git_bytes(path, W05_R01_BASE)\n    assert hashlib.sha256(before).hexdigest() == "6d04e15cdf30e3a832af1d50fdfa1bec85577710910817100aaa110fd7021169"\n    changed = {\n        "test_each_unit_has_exact_four_common_records_and_approved_paths",\n        "test_w04_r01_name_is_cumulative_but_immediate_permissions_are_not",\n        "test_w04_r01_driver_keeps_original_p2_and_unrelated_ci_ast",\n    }\n    live = ast.parse((ROOT / path).read_bytes())\n    functions = [n for n in live.body if isinstance(n, ast.FunctionDef)]\n    original = [n for n in ast.parse(before).body if isinstance(n, ast.FunctionDef)]\n    for node in original:\n        current = [n for n in functions if n.name == node.name]\n        assert len(current) == 1, node.name\n        if node.name not in changed:\n            assert ast.dump(node) == ast.dump(current[0]), node.name\n        else:\n            assert ast.dump(node) != ast.dump(current[0]), node.name\n    assert changed <= {n.name for n in original}', 'def test_w05_r01_only_three_old_transition_methods_have_authorized_adaptations():\n    path = "tests/contract/test_phase3_transition.py"\n    before = git_bytes(path, W05_R01_BASE)\n    assert hashlib.sha256(before).hexdigest() == "6d04e15cdf30e3a832af1d50fdfa1bec85577710910817100aaa110fd7021169"\n    changed = {\n        "test_each_unit_has_exact_four_common_records_and_approved_paths",\n        "test_w04_r01_name_is_cumulative_but_immediate_permissions_are_not",\n        "test_w04_r01_driver_keeps_original_p2_and_unrelated_ci_ast",\n    }\n    live = ast.parse(git_bytes(path, W06_R01_PREDECESSOR))\n    functions = [n for n in live.body if isinstance(n, ast.FunctionDef)]\n    original = [n for n in ast.parse(before).body if isinstance(n, ast.FunctionDef)]\n    for node in original:\n        current = [n for n in functions if n.name == node.name]\n        assert len(current) == 1, node.name\n        if node.name not in changed:\n            assert ast.dump(node) == ast.dump(current[0]), node.name\n        else:\n            assert ast.dump(node) != ast.dump(current[0]), node.name\n    assert changed <= {n.name for n in original}\n    checkpoint = git_bytes(path, W09_R01_BASE)\n    assert hashlib.sha256(checkpoint).hexdigest() == "fb67b75edab831c9fc96162eada7c760c07c8f9d0eadf1e30b06cb4934198285"\n    expected = checkpoint.decode()\n    for previous, replacement in ((\'def test_each_unit_has_exact_four_common_records_and_approved_paths(step):\\n    paths = guard.phase3_plan_paths(ROOT)\\n    ci = ci_driver()\\n    unit = f"P3-W{step:02}"\\n    expected = COMMON | ADDITIONAL[step]\\n    assert set(paths) == {f"P3-W{number:02}" for number in range(1, 16)}\\n    assert paths[unit] == expected\\n    effective = expected | (W04_R01_PATHS if step == 4 else frozenset())\\n    if step == 5:\\n        effective |= W05_R01_PATHS\\n    if step == 6:\\n        effective |= W06_R01_PATHS\\n    if step == 8:\\n        effective |= W08_R01_PATHS\\n    assert ci.phase3_effective_paths(paths, unit) == effective\\n    ci.phase3_check_changed_paths(paths, unit, effective)\\n    cumulative = COMMON | frozenset().union(*(ADDITIONAL[number] for number in range(1, step + 1)))\\n    if step >= 4:\\n        cumulative |= W04_R01_PATHS\\n    if step >= 5:\\n        cumulative |= W05_R01_PATHS\\n    if step >= 6:\\n        cumulative |= W06_R01_PATHS\\n    if step >= 8:\\n        cumulative |= W08_R01_PATHS\\n    assert ci.phase3_effective_paths(paths, unit, cumulative=True) == cumulative\\n    assert len(COMMON) == 4\\n    for future_path in (COMMON | frozenset().union(*ADDITIONAL.values())) - effective:\\n        with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\\n            ci.phase3_check_changed_paths(paths, unit, {future_path})\', \'def test_each_unit_has_exact_four_common_records_and_approved_paths(step):\\n    paths = guard.phase3_plan_paths(ROOT)\\n    ci = ci_driver()\\n    unit = f"P3-W{step:02}"\\n    expected = COMMON | ADDITIONAL[step]\\n    assert set(paths) == {f"P3-W{number:02}" for number in range(1, 16)}\\n    assert paths[unit] == expected\\n    effective = expected | (W04_R01_PATHS if step == 4 else frozenset())\\n    if step == 5:\\n        effective |= W05_R01_PATHS\\n    if step == 6:\\n        effective |= W06_R01_PATHS\\n    if step == 8:\\n        effective |= W08_R01_PATHS\\n    if step == 9:\\n        effective |= W09_R01_PATHS\\n        assert len(expected) == 7 and len(effective) == 10\\n        assert effective - expected == W09_R01_PATHS\\n    assert ci.phase3_effective_paths(paths, unit) == effective\\n    ci.phase3_check_changed_paths(paths, unit, effective)\\n    cumulative = COMMON | frozenset().union(*(ADDITIONAL[number] for number in range(1, step + 1)))\\n    if step >= 4:\\n        cumulative |= W04_R01_PATHS\\n    if step >= 5:\\n        cumulative |= W05_R01_PATHS\\n    if step >= 6:\\n        cumulative |= W06_R01_PATHS\\n    if step >= 8:\\n        cumulative |= W08_R01_PATHS\\n    if step >= 9:\\n        cumulative |= W09_R01_PATHS\\n    assert ci.phase3_effective_paths(paths, unit, cumulative=True) == cumulative\\n    assert len(COMMON) == 4\\n    for future_path in (COMMON | frozenset().union(*ADDITIONAL.values())) - effective:\\n        with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\\n            ci.phase3_check_changed_paths(paths, unit, {future_path})\'), (\'def test_w04_r01_literal_authority_preserves_plan_guard_and_workflow():\\n    ci = ci_driver()\\n    assert ci.P3_W04_R01_PATHS == W04_R01_PATHS and len(W04_R01_PATHS) == 4\\n    assert ci.P3_W04_R01_BASE == W04_R01_BASE\\n    assert ci.P3_W04_R01_BASE_TREE == W04_R01_BASE_TREE\\n    assert ci.P3_W04_R01_PREDECESSOR == W04_R01_PREDECESSOR\\n    paths = guard.phase3_plan_paths(ROOT)\\n    assert len(paths["P3-W04"]) == 13\\n    assert len(ci.phase3_effective_paths(paths, "P3-W04")) == 17\\n    assert len(ci.phase3_effective_paths(paths, "P3-W01")) == 23\\n    # Original W01 permission already contains these paths; R01 gives no new\\n    # retrospective W01 permission and cannot enlarge raw plan parsing.\\n    assert W04_R01_PATHS <= paths["P3-W01"]\\n    assert not W04_R01_PATHS & paths["P3-W04"]\\n    for path, digest in (\\n        ("tools/check_scaffold_boundary.py", "fc9885337b4973a084c4c7326370a7962b898c14f6a0858e09ee6ccbeb97071a"),\\n        (".github/workflows/phase1-ci.yml", "841c39f7d3c596eafd767355a4a30f5111d6a73c8752229108708fcb39c4539e"),\\n        ("PHASE_3_PLAN.md", PLAN_SHA256),\\n        ("phase3/entry_manifest.json", "dfe96f57a222937131e8e4fafd13098f76d134cd88a50ffa9fa82891b9b9659b"),\\n    ):\\n        assert hashlib.sha256((ROOT / path).read_bytes()).hexdigest() == digest, path\', \'def test_w04_r01_literal_authority_preserves_plan_guard_and_workflow():\\n    ci = ci_driver()\\n    assert ci.P3_W04_R01_PATHS == W04_R01_PATHS and len(W04_R01_PATHS) == 4\\n    assert ci.P3_W04_R01_BASE == W04_R01_BASE\\n    assert ci.P3_W04_R01_BASE_TREE == W04_R01_BASE_TREE\\n    assert ci.P3_W04_R01_PREDECESSOR == W04_R01_PREDECESSOR\\n    paths = guard.phase3_plan_paths(ROOT)\\n    assert len(paths["P3-W04"]) == 13\\n    assert len(ci.phase3_effective_paths(paths, "P3-W04")) == 17\\n    assert len(ci.phase3_effective_paths(paths, "P3-W01")) == 23\\n    # Original W01 permission already contains these paths; R01 gives no new\\n    # retrospective W01 permission and cannot enlarge raw plan parsing.\\n    assert W04_R01_PATHS <= paths["P3-W01"]\\n    assert not W04_R01_PATHS & paths["P3-W04"]\\n    for path, digest in (\\n        ("tools/check_scaffold_boundary.py", "fc9885337b4973a084c4c7326370a7962b898c14f6a0858e09ee6ccbeb97071a"),\\n        (".github/workflows/phase1-ci.yml", "841c39f7d3c596eafd767355a4a30f5111d6a73c8752229108708fcb39c4539e"),\\n        ("PHASE_3_PLAN.md", PLAN_SHA256),\\n        ("phase3/entry_manifest.json", "dfe96f57a222937131e8e4fafd13098f76d134cd88a50ffa9fa82891b9b9659b"),\\n    ):\\n        actual = (ROOT / path).read_bytes()\\n        if path == ".github/workflows/phase1-ci.yml":\\n            historical = git_bytes(path, W09_R01_BASE)\\n            assert hashlib.sha256(historical).hexdigest() == digest, path\\n            assert historical.count(b\\\'      "timeout-minutes": 25,\\\\n\\\') == 1\\n            assert actual == historical.replace(b\\\'      "timeout-minutes": 25,\\\\n\\\', b\\\'      "timeout-minutes": 40,\\\\n\\\', 1)\\n        else:\\n            assert hashlib.sha256(actual).hexdigest() == digest, path\'), (\'def test_w04_r01_name_is_cumulative_but_immediate_permissions_are_not(step):\\n    ci = ci_driver()\\n    unit = f"P3-W{step:02}"\\n    paths = guard.phase3_plan_paths(ROOT)\\n    assert ci.phase3_scope_exceptions(unit) == ((["P3-W04-R01"] if step >= 4 else []) +\\n                                              (["P3-W05-R01"] if step >= 5 else []) +\\n                                              (["P3-W06-R01"] if step >= 6 else []) +\\n                                              (["P3-W08-R01"] if step >= 8 else []))\\n    for path in W04_R01_PATHS:\\n        # Overlapping paths require their own approved immediate-unit scope.\\n        if (step in (1, 4) or (step == 5 and path in W05_R01_PATHS) or\\n                (step == 6 and path in W06_R01_PATHS) or (step == 8 and path in W08_R01_PATHS)):\\n            ci.phase3_check_changed_paths(paths, unit, {path})\\n        else:\\n            with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\\n                ci.phase3_check_changed_paths(paths, unit, {path})\\n    # All four were in W01 already; the cumulative union is still independently\\n    # determined by approved scopes, with no new caller/source metadata input.\\n    assert W04_R01_PATHS <= ci.phase3_effective_paths(paths, unit, cumulative=True)\', \'def test_w04_r01_name_is_cumulative_but_immediate_permissions_are_not(step):\\n    ci = ci_driver()\\n    unit = f"P3-W{step:02}"\\n    paths = guard.phase3_plan_paths(ROOT)\\n    assert ci.phase3_scope_exceptions(unit) == ((["P3-W04-R01"] if step >= 4 else []) +\\n                                              (["P3-W05-R01"] if step >= 5 else []) +\\n                                              (["P3-W06-R01"] if step >= 6 else []) +\\n                                              (["P3-W08-R01"] if step >= 8 else []) +\\n                                              (["P3-W09-R01"] if step >= 9 else []))\\n    for path in W04_R01_PATHS:\\n        # Overlapping paths require their own approved immediate-unit scope.\\n        if (step in (1, 4) or (step == 5 and path in W05_R01_PATHS) or\\n                (step == 6 and path in W06_R01_PATHS) or (step == 8 and path in W08_R01_PATHS) or\\n                (step == 9 and path in W09_R01_PATHS)):\\n            ci.phase3_check_changed_paths(paths, unit, {path})\\n        else:\\n            with pytest.raises(ValueError, match="^work_unit_allowlist_exceeded$"):\\n                ci.phase3_check_changed_paths(paths, unit, {path})\\n    # All four were in W01 already; the cumulative union is still independently\\n    # determined by approved scopes, with no new caller/source metadata input.\\n    assert W04_R01_PATHS <= ci.phase3_effective_paths(paths, unit, cumulative=True)\')):\n        assert expected.count(previous) == 1, "invalid_fixed_w09_transition_delta"\n        expected = expected.replace(previous, replacement, 1)\n    expected_functions = {n.name: n for n in ast.parse(expected).body if isinstance(n, ast.FunctionDef)}\n    current_functions = {n.name: n for n in ast.parse((ROOT / path).read_bytes()).body if isinstance(n, ast.FunctionDef)}\n    for node in original:\n        assert ast.dump(current_functions[node.name]) == ast.dump(expected_functions[node.name]), node.name')):
        assert expected.count(prior) == 1, "invalid_fixed_w09_transition_delta"
        expected = expected.replace(prior, replacement, 1)
    original_names = {node.name for node in previous if isinstance(node, ast.FunctionDef)}
    for method, prior, replacement in _W11_R01_TRANSITION_DELTAS:
        if method in original_names:
            node = next(node for node in ast.parse(expected).body
                        if isinstance(node, ast.FunctionDef) and node.name == method)
...
# Independently transcribed approved P3-W09-R01 fixed boundary and exact scope.
W09_R01_PATHS = frozenset((
    "tests/scaffold/test_ci_contract.py",
    "tests/contract/test_phase3_transition.py",
    ".github/workflows/phase1-ci.yml",
))
W09_R01_BASE = "5bccceee13995f7ee51ed44339d596aadb309644"
W09_R01_BASE_TREE = "012e520e14977a3ee6a6a1ab143799cfa87ca18a"
W09_R01_PREDECESSOR = "c25c827b9c0c2cf84075c79ebbf6c238dee4ca74"


# Independently transcribed owner-approved P3-W11-R01: exact Assertion before-target dependency repair.
# The original six-path checkpoint remains outside the approved successor scope.
W11_R01_PATHS = frozenset((
    "src/source_integrity_toolkit/contracts/results.py",
    "tests/scaffold/test_ci_contract.py",
    "tests/contract/test_phase3_transition.py",
))
W11_R01_BASE = "48c9a22e2b07df545cd492f921604431c3fec09c"
W11_R01_BASE_TREE = "e1eb494e80e00d5d99640704a81ae4399cd12a86"
W11_R01_PREDECESSOR = "82dc7de63e1007f007e527f53ef0ba719e9c1db4"
